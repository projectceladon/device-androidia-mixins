...
ifneq (,$(filter cic cic_dev,$(TARGET_PRODUCT)))
	...
	$(hide) rm -rf $(PRODUCT_OUT)/cfc $(PRODUCT_OUT)/pre-requisites  $(PRODUCT_OUT)/README-CIC  $(PRODUCT_OUT)/setup-aic $(PRODUCT_OUT)/INFO.pdf
...
ifneq (,$(filter cic cic_dev,$(TARGET_PRODUCT)))
	tar cvzf $(PRODUCT_OUT)/$(TARGET_AIC_FILE_NAME) -C $(PRODUCT_OUT) aic android.tar.gz aic-manager.tar.gz cfc pre-requisites README-CIC INFO.pdf cic.sh setup-aic tos.img kf4cic.efi -C docker update
	...
	$(hide) (cd $(PRODUCT_OUT)/cic/opt/cic && tar xvf ../../../$(TARGET_AIC_FILE_NAME) aic android.tar.gz aic-manager.tar.gz INFO.pdf cic.sh cfc update)

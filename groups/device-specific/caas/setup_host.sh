...
function ubu_enable_host_gvtd(){
	...
	if [[ ! `cat /etc/default/grub` =~ "intel_iommu=on kvm-intel.nested=y i915.force=probe=* " ]]; then
		read -p "The grub entry in '/etc/default/grub' will be updated for enabling GVT-d, do you want to continue? [Y/n]" res
		if [ x$res = xn ]; then
			exit 0
		fi
		sed -i "s/GRUB_CMDLINE_LINUX=\"/GRUB_CMDLINE_LINUX=\"intel_iommu=on kvm-intel.nested=y i915.force_probe=* /g" /etc/default/grub
		update-grub
		reboot_required=1
	fi
}

...
RPMB_DATA=${work_dir}/RPMB_DATA
RPMB_SOCK=${work_dir}/rpmb_sock
...
function setup_rpmb(){
	...
	${RPMB_DEV} --dev ${RPMB_DATA} --sock ${RPMB_SOCK} &
	RPMB_DEV_PID=$!
}
...
function cleanup_rpmb_sock(){
	# Send kill signal to rpmb_dev process, remove RPMB sock
	if [[ ! -z "$RPMB_DEV_PID" ]]; then
		kill -9 $RPMB_DEV_PID
		rm ${RPMB_SOCK}
	fi
}
